...
class CinnamonCalendarApplet extends Applet.TextApplet {
    constructor(orientation, panel_height, instance_id) {
        ...
        try {
            ...
            // Events
            this.events_manager = new EventView.EventsManager(
                this.settings,
                this.desktop_settings,
            );
            this.events_manager.connect(
                "events-manager-ready",
                this._events_manager_ready.bind(this),
            );
            this.events_manager.connect(
                "has-calendars-changed",
                this._has_calendars_changed.bind(this),
            );
            ...
            this.go_home_button.connect(
                "enter-event",
                Lang.bind(this, (actor, event) => {
            ...
            // ★ 创建农历头部容器（将显示在“星期三 / 日期”下方）
            let lunarHeaderContainer = new St.BoxLayout({
                vertical: false,
                x_expand: true, // 扩展以占满宽度
                x_align: Clutter.ActorAlign.CENTER, // 内容居中
            });
            calbox.add_actor(lunarHeaderContainer);
            ...
            this._calendar.setEventsSection(lunarHeaderContainer); // 关键：将农历头部放入容器
            ...
            this.actor.connect(
                "enter-event",
                Lang.bind(this, function (menu, event) {
    ...
    _updateClockAndDate() {
        ...
        let dateFormattedTooltip = this.clock
            .get_clock_for_format(DATE_FORMAT_FULL)
            .capitalize();
        if (this.use_custom_format) {
            dateFormattedTooltip = this.clock
                .get_clock_for_format(this.custom_tooltip_format)
                .capitalize();
            if (!dateFormattedTooltip) {
                global.logError(
                    "Calendar applet: bad tooltip time format string - check your string.",
                );
                dateFormattedTooltip = this.clock.get_clock_for_format(
                    "~CLOCK FORMAT ERROR~ %l:%M %p",
                );
            }
        }

        let dateFormattedShort = this.clock
            .get_clock_for_format(DATE_FORMAT_SHORT)
            .capitalize();
        let dayFormatted = this.clock
            .get_clock_for_format(DAY_FORMAT)
            .capitalize();
